...
st.write("Select a tool from the options below to get started.")

# --- 3. Create Buttons on the Main Screen ---
# Use columns to arrange the buttons nicely
cols = st.columns(len(TOOL_OPTIONS))

for i, tool_name in enumerate(TOOL_OPTIONS):
    with cols[i]:
        # Create a button for each tool. 
        # The key is important for Streamlit to track the button state.
        if st.button(tool_name, key=f"button_{i}"):
            # When a button is clicked, update the session state
            st.session_state.selected_tool = tool_name
            # Rerun the script to display the new content
            st.rerun() 
...
# 1) Name & Address Match (your script)
    ...
    # 2) URL Scraper & Fuzzy Matching
        ...
        # Check ddgs availability (Uncomment the try/except block once DDGS is installed)
        # try:
        #     DDGS  # noqa: F841
        # except Exception:
        ...
        if uploaded_file is not None and script_option != "Select":
            ...
            # --- SCRIPT 1: URL Scraper Configuration and Logic ---
            if script_option == "Script 1: URL Scraper":
                ...
                # Container for Run/Stop/Resume/Download buttons
                run_btn = st.button("Run URL Script", key="url_run")

                # Status updates (placeholders outside the processing loop)
                progress_text = st.empty()
                progress_bar = st.empty()
                
                # Stop, Resume, and Download Buttons
                ...
                # RESUME button (visible only after stop and rows remain)
                ...
                if (not st.session_state.url_running) and st.session_state.url_stop and st.session_state.url_index < len(df):
                    with resume_col:
                        if st.button("Resume URL Script", key="resume_url_script"):
                            ...
                            st.rerun() # Rerun to start the processing loop immediately
                
                # DOWNLOAD button (visible if rows have been processed AND not actively running or is stopped)
                if st.session_state.url_rows and (not st.session_state.url_running or st.session_state.url_stop):
                    with dl_col:
                        processed_df = pd.DataFrame(st.session_state.url_rows)
                        towrite = io.BytesIO()
                        processed_df.to_excel(towrite, index=False, engine="openpyxl")
                        towrite.seek(0)
                        st.download_button(
                            "Download Processed Rows (XLSX)", 
                            data=towrite.getvalue(), 
                            file_name="url_processed_rows.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key="download_midway"
                        )
                ...
                # constants (using widget values
                VERIFY_CANDIDATES = verify_fetch
                MAX_RESULTS_DDG = max_results
                FETCH_TIMEOUT = 8
                PHONE_BOOST = 20
                ADDRESS_BOOST = 15
                DOMAIN_PENALTY = 30
                ...
                ALWAYS_UNOFFICIAL_DOMAINS = {'webmd.com', 'www.webmd.com', 'npiregistry.cms.hhs.gov', 'nppes.cms.hhs.gov', 'npiregistry', 'npi.registry', 'npi-registry', 'healthgrades.com', 'www.healthgrades.com', 'health.usnews.com', 'www.health.usnews.com', 'npidb.com', 'www.npidb.com'}
                blocked_domains = {'www.mapquest.com', 'www.blockedsite.org', 'spam-site.com'}
                blocked_urls = {'https://specific-url-to-ignore.com/path', 'https://another-url.com'}
                ...
                if run_btn:
                    ...
                    st.session_state.url_rows = []
                    st.session_state.url_total = len(df)
                    st.rerun() # Use rerun to immediately start the process
                ...
                if st.session_state.url_running:
                    ...
                    # Initialize UI elements inside the running block for persistence
                    progress_bar_widget = progress_bar.progress(int(start_idx/total*100))
                    ...
                    for idx in range(start_idx, total):
                        ...
                        if st.session_state.url_stop:
                            ...
                            st.session_state.url_running = False
                            st.rerun() # IMPORTANT: Rerun to update the button and download state
                            break
                        ...
                        progress_bar_widget.progress(int((idx+1)/total*100))
                ...
                # Final Results Display (Runs after loop completes or stops)
                if (not st.session_state.url_running) and st.session_state.url_rows:
                    if st.session_state.url_index == st.session_state.url_total:
                        st.success("✅ Script finished.")
                    
                    result_df = pd.DataFrame(st.session_state.url_rows)
                    st.dataframe(result_df.head(200))

                    # Final Download Button (separate from the mid-way download)
                    towrite = io.BytesIO()
                    result_df.to_excel(towrite, index=False, engine="openpyxl")
                    towrite.seek(0)
                    st.download_button("Download Final Results (XLSX)", towrite.read(), file_name="url_final_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    #vid number extraction
    ...
    elif st.session_state.selected_tool == "Vid Number Extraction":
        ...
        """, unsafe_allow_html=True)
        st.header(" Upload the network and the user file,"
                "Then select the necessary columns for the successful vid extraction")
